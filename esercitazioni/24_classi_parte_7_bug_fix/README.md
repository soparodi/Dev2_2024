...
- Correggere il bug che impedisce di visualizzare i prodotti tramite 1. visualizza , attualmente visualizza solo i prodotti aggiunti in runtime ma non quelli del file JSON
...
- Correggere il bug che continua ad incrementare l'id in lettura e in scrittura
...
public class ProdottoAdvanced
{
    ...
    public int Id { set; get; } = GetLastId();
    ...
    static int GetLastId()
    {
        ...
        {
            ProdottoRepository repository = new ProdottoRepository();
            List<ProdottoAdvanced> list = repository.CaricaProdotti();
            int max = 0;
            foreach (ProdottoAdvanced item in list)
            {
                if (item.Id > max)
                {
                    max = item.Id;
